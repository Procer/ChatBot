...
async def api_generate_client_proposal_pdf(client_id: int, precio_ars: float, mensaje: str | None = None, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    return Response(
        ...
        headers={"Content-Disposition": f'inline; filename="Cobro_Chatbot_{client.slug}.pdf"'},
    )


@app.get("/api/superadmin/clients/{client_id:int}/proposal-image")
async def api_generate_client_proposal_image(client_id: int, precio_ars: float, mensaje: str | None = None, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    if not require_superadmin(current_user):
        return JSONResponse(status_code=401, content={"error": "No autorizado"})
    client = db.query(Client).filter_by(id=client_id).first()
    if not client:
    ...
    return Response(
        ...
        headers={"Content-Disposition": f'inline; filename="Cobro_Chatbot_{client.slug}.png"'},
    )
